...
const descargarArchivo = (id) => {
    bulmaToast.toast({ message: 'Descargando Archivo!', type: 'is-success', position: 'bottom-right', animate: { in: 'fadeIn', out: 'fadeOut' }})
        ...
        const cargarexp = (elemento) =>{
            ...
            Tipo_exp = query;
            let IdProp = document.getElementById('IdUsuario').value;
            console.log(IdProp);
            //Fetch de requisitos del tipo de expediente seleccionado
            fetch('/expediente/fetch/prop/' + query +'/' +  IdProp, {
                method: 'GET',
                headers: {
                ...
                if(data == 'fail'){
                    ...
                    for(elements of data){
                        ...
                        let filelink = (elements.estatus != 0) ? `/expediente/downloadProp/${IdProp}/${elements.tipo_doc}/${Tipo_exp}` : 'javascript:void(0)';
        ...
        const actualizarExp = () => {
            ...
            fetch('/expediente/actualizarProp', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
